...
func (h *Handler) AddUser(c *gin.Context) {
	h.Log.Info("AddUser handler is invoked")
	...
	resp, err := h.AdminClient.AddUser(ctx, &req)
	if err != nil {
		er := errors.Wrap(err, "error adding user").Error()
		c.AbortWithStatusJSON(http.StatusInternalServerError, gin.H{"error": er})
		h.Log.Error(er)
		return
	}

	h.Log.Info("AddUser handler is completed")
...
// @Success 200 {object} admin.UserInfo
...
func (h *Handler) GetUser(c *gin.Context) {
	...
	resp, err := h.AdminClient.GetUser(ctx, &pb.ID{Id: id})
	if err != nil {
...
// @Security ApiKeyAuth
// @Param id path string true "User ID"
// @Param user body admin.NewDataNoID true "New user data"
// @Success 200 {object} admin.NewDataResp
// @Failure 400 {object} string "Invalid user id"
// @Failure 500 {object} string "Server error while processing request"
...
func (h *Handler) UpdateUser(c *gin.Context) {
	...
	resp, err := h.AdminClient.UpdateUser(ctx, &pb.NewData{
		Id:          id,
...
func (h *Handler) DeleteUser(c *gin.Context) {
	...
	_, err = h.AdminClient.DeleteUser(ctx, &pb.ID{Id: id})
	if err != nil {
...
// @Success 200 {object} admin.Users
...
func (h *Handler) FetchUsers(c *gin.Context) {
	...
	resp, err := h.AdminClient.FetchUsers(ctx, &pb.Filter{
		FullName: name,
		Location: loc,
		Role:     role,
		Page:     int32(p),
