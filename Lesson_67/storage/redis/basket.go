...
func (b *BasketRepo) AddProduct(ctx context.Context, req *pb.NewProduct, price float32) error {
	basketKey := "basket:" + req.UserId
	basket, err := b.GetBasket(ctx, &pb.Id{UserId: req.UserId})
	if err != nil {
...
func (b *BasketRepo) GetBasket(ctx context.Context, id *pb.Id) (*models.Basket, error) {
	basketKey := "basket:" + id.UserId
...
func (b *BasketRepo) DeleteProduct(ctx context.Context, req *pb.Ids) error {
	basketKey := "basket:" + req.UserId
	basket, err := b.GetBasket(ctx, &pb.Id{UserId: req.UserId})
	if err != nil {
		return errors.Wrap(err, "basket retrieval failure")
