...
func (h *Handler) Register(c *gin.Context) {
	...
	resp, err := h.Storage.Admin().Add(ctx, req)
	if err != nil {
...
func (h *Handler) Login(c *gin.Context) {
	...
	id, username, passwordHash, err := h.Storage.User().GetUserByEmail(ctx, req.Email)
	if err != nil {
	...
	err = h.Storage.Token().Store(ctx, &models.RefreshTokenDetails{
		UserID: id,
		Token:  refreshToken,
		Expiry: exp,
	})
...
func (h Handler) Refresh(c *gin.Context) {
	...
	username, email, _, err := h.Storage.User().GetUserByID(ctx, id)
	if err != nil {
		er := errors.Wrap(err, "user not found").Error()
		c.AbortWithStatusJSON(http.StatusBadRequest,
			gin.H{"error": er},
...
func (h *Handler) Logout(c *gin.Context) {
	...
	err = h.Storage.Token().Delete(ctx, UserId)
	if err != nil {
